...
class ZQSeparateMAC():
    ...
    def forward_agent(self, data, idx):  # should only be used in training
        z_q_vals = self._forward_z_q(data, idx, data["cur_z_p"], train=True)
    ...
    def select_z_agent(self, data, t_env, idx, test_mode=False):
        # z_p
        z_p_outs, z_q_outs = self.forward_agent(data, idx)
        z_p_outs = z_p_outs.view(1, self.args.latent_relation_space_dim, -1)
        z_q_outs = z_q_outs.view(1, self.args.latent_relation_space_dim, -1)
