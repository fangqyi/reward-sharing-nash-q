...
class ZACSeparateMAC:
    ...
    def select_z(self, data, t_env, test_mode=False):
        return self.forward(data, train=False)

    def select_z_agent(self, data, t_env, idx, test_mode=False):
        return self.forward_agent(data, idx, train=False)

    def forward(self, data):
        z_p, z_q, log_prob_z_p, log_prob_z_q, prob_z_p, prob_z_q = [], [], [], [], [], []
        for i in range(self.n_agents):
            z_p_i, z_q_i, log_prob_z_p_i, log_prob_z_q_i, prob_z_p_i, prob_z_q_i = self.forward_agent(data, i)
            z_p.append(z_p_i)
            z_q.append(z_q_i)
            log_prob_z_p.append(log_prob_z_p_i)
            log_prob_z_q.append(log_prob_z_q_i)
        ...
        return (th.stack(z_p, dim=0).view(1, self.args.n_agents, self.args.latent_relation_space_dim),
                ...
                th.stack(log_prob_z_p, dim=0),
                th.stack(log_prob_z_q, dim=0),
    ...
    def forward_agent(self, data, idx):
        ...
        return (self.z_p_actors[idx].z.detach(),
                self.z_q_actors[idx].z.detach(),
                self.z_p_actors[idx].log_prob_z,
                self.z_q_actors[idx].log_prob_z,
                self.z_p_actors[idx].prob_z,
...
class ZACDiscreteSeparateMAC(ZACSeparateMAC):
    ...
    def forward_agent(self, data, idx):
        z_p_inputs = self._build_z_p_input(data)
        z_p_idx, log_prob_z_p, prob_z_p = self.z_p_actors[idx].sample(z_p_inputs)
        ...
        z_q_idx, log_prob_z_q, prob_z_q = self.z_q_actors[idx].sample(z_q_inputs)
        ...
        return z_p, z_q, log_prob_z_p, log_prob_z_q, prob_z_p, prob_z_q
