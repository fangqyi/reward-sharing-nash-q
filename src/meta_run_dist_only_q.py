...
def run_distance_sequential(args, logger):
    ...
    z_buffers = [ReplayBuffer(z_scheme, groups, args.z_buffer_size, max_seq_length=z_max_seq_length, device=device)
    ...
    while z_train_steps <= args.total_z_training_steps:
        ...
        for i in range(args.n_agents):
            ...
            if z_buffers[i].can_sample(args.z_batch_size):
                z_sample = z_buffers[i].sample(args.z_batch_size)
