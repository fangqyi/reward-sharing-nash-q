...
def run_sequential(args, logger):
    ...
    scheme = {
        ...
        "p": {"vshape": (args.n_agents,), "groups": "agents", "dtype": th.long},
        "q": {"vshape": (args.n_agents,), "groups": "agents", "dtype": th.long},
        "pq": {"vshape": (2,), "group": "agents", "dtype": th.long},
...
def run_pq_sequential(args, logger):
    # Init runner so we can get env info
    runner = r_REGISTRY[args.runner](args=args, logger=logger)
    ...
    scheme = {
        ...
        "obs": {"vshape": env_info["obs_shape"], "group": "agents"},
        "actions": {"vshape": (1,), "group": "agents", "dtype": th.long},
        "p": {"vshape": (env_info["n_agents"],), "group": "agents", "dtype": th.long},
        "q": {"vshape": (env_info["n_agents"],), "group": "agents", "dtype": th.long},
        "pq": {"vshape": (2,), "group": "agents", "dtype": th.long},
        "avail_actions": {"vshape": (env_info["n_actions"],), "group": "agents", "dtype": th.int},
        "rewards": {"vshape": env_info["reward_shape"], },
        "redistributed_rewards": {"vshape": env_info["reward_shape"], },
    ...
    pq_training_cnt = 0
    env_steps_every_pq_max = 0
    while pq_training_cnt <= args.max_pq_training_steps:
        device = "cpu" if args.buffer_cpu_only else args.device
        buffer = ReplayBuffer(scheme, groups, args.buffer_size, env_info["episode_limit"] + 1,
                              preprocess=preprocess,
                              device=device)
        p, q = mac.select_pqs(batch_size=1, device=device, eps_num=pq_training_cnt, test_mode=False)
        env_steps_every_pq_max += args.env_steps_every_pq
        while runner.t_env <= env_steps_every_pq_max:
            # Run for a whole episode at a time
            episode_batch = runner.run(p, q, test_mode=False)
            buffer.insert_episode_batch(episode_batch)
